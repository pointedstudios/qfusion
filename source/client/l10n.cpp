...
static std::string L10n_CombineFileName( const std::string &filePath,
										 const std::string &lang,
										 std::string::size_type numLangChars = std::string::npos ) {
	if( numLangChars == std::string::npos ) {
		numLangChars = lang.size();
	}
	std::string result;
	result.reserve( filePath.size() + 1 + numLangChars + 3 );
	result += filePath;
	if( filePath.back() != '/' ) {
		result += '/';
	}
	result.append( lang.c_str(), numLangChars );
	result += ".po";
	return result;
}
...
*/
static bool L10n_LoadLangPOFile_( podomain_t *podomain, const std::string &filePath, const std::string &lang ) {
	// The code was fishy and fixing it for compiling as C++
	// due to lack of const-correctness was impossible so we rewrote it.

	std::string tempFileName( L10n_CombineFileName( filePath, lang ) );
	pofile_t *file = L10n_FindPOFile( podomain, tempFileName.c_str() );
	if( file ) {
		return true;
	}

	podict_t *dict = L10n_LoadPODict( tempFileName.c_str() );
	if( !dict ) {
		// Try loading a country-independent file
		auto underscoreIndex = lang.find_first_of( '_' );
		if( underscoreIndex != std::string::npos ) {
			std::string tempFileName2( L10n_CombineFileName( filePath, lang, underscoreIndex ) );
			dict = L10n_LoadPODict( tempFileName2.c_str() );
		}
	}

	if( dict ) {
		file = L10n_CreatePOFile( tempFileName.c_str() );
		file->dict = dict;
		file->next = podomain->pofiles_head;
		podomain->pofiles_head = file;
	}
...
void L10n_LoadLangPOFile( const char *domainname, const char *filepath ) {
	...
	podomain_t *podomain = L10n_FindPODomain( domainname );
	if( !podomain ) {
	...
	std::string lang( L10n_GetUserLanguage() );
	std::string filePath_( filepath );
	if( !L10n_LoadLangPOFile_( podomain, filePath_, lang ) ) {
		std::string defaultLang( APP_DEFAULT_LANGUAGE );
		// load default lang .po file
		if( lang == defaultLang ) {
			L10n_LoadLangPOFile_( podomain, filePath_, defaultLang );
		}
	}
}
