...
static void CG_DrawEntityNumbers( void ) {
	...
	for( i = 0; i < cg.frame.numEntities; i++ ) {
		...
		if( trace.fraction == 1.0f ) {
			...
			SCR_DrawString( coords[0] + shadowOffset, coords[1] + shadowOffset,
								 ALIGN_LEFT_MIDDLE, va( "%i", cent->current.number ), cgs.fontSystemSmall, colorBlack );
			SCR_DrawString( coords[0], coords[1],
...
void CG_Democam_DrawCenterSubtitle( int y, unsigned int maxwidth, struct qfontface_s *font, char *text ) {
	...
	if( !maxwidth || SCR_strWidth( text, font, 0 ) <= maxwidth ) {
		SCR_DrawStringWidth( x + shadowOffset, y + shadowOffset, ALIGN_CENTER_TOP, COM_RemoveColorTokens( text ), maxwidth, font, colorBlack );
		SCR_DrawStringWidth( x, y, ALIGN_CENTER_TOP, text, maxwidth, font, colorWhite );
		return;
	}
	...
	while( *s ) {
		...
		if( ( !*s || *s == '\n' ) && SCR_strWidth( ptr, font, 0 ) < maxwidth ) { // new line or end of text, in both cases force write
			c = *s;
			*s = 0;
			SCR_DrawStringWidth( x + shadowOffset, y + shadowOffset, ALIGN_CENTER_TOP, COM_RemoveColorTokens( ptr ), maxwidth, font, colorBlack );
			SCR_DrawStringWidth( x, y, ALIGN_CENTER_TOP, ptr, maxwidth, font, colorWhite );
			*s = c;
			...
			*s = 0;
			...
			if( SCR_strWidth( ptr, font, 0 ) < maxwidth ) {
				*s = c;
				t = s;
				s++;
				continue;
...
void CG_DrawDemocam2D( void ) {
	...
	if( democam_editing_mode ) {
		...
		if( *cgs.demoName ) {
			SCR_DrawString( xpos, ypos, ALIGN_LEFT_TOP, va( "Demo: %s", cgs.demoName ), cgs.fontSystemSmall, colorWhite );
			ypos += SCR_FontHeight( cgs.fontSystemSmall );
		}

		SCR_DrawString( xpos, ypos, ALIGN_LEFT_TOP, va( "Play mode: %s%s%s", S_COLOR_ORANGE, CamIsFree ? "Free Fly" : "Preview", S_COLOR_WHITE ), cgs.fontSystemSmall, colorWhite );
		ypos += SCR_FontHeight( cgs.fontSystemSmall );

		SCR_DrawString( xpos, ypos, ALIGN_LEFT_TOP, va( "Time: %" PRIi64, demo_time ), cgs.fontSystemSmall, colorWhite );
		ypos += SCR_FontHeight( cgs.fontSystemSmall );
		...
		SCR_DrawString( xpos, ypos, ALIGN_LEFT_TOP,
...
#undef VectorHermiteInterp
			...
			case DEMOCAM_ORBITAL:
				...
				if( !currentcam->trackEnt || currentcam->trackEnt >= MAX_EDICTS ) {
					...
					// if having a bounding box, look to its center
					if( ( cmodel = CG_CModelForEntity( currentcam->trackEnt ) ) ) {
						vec3_t mins, maxs;
						CG_InlineModelBounds( cmodel, mins, maxs );
						for( i = 0; i < 3; i++ )
