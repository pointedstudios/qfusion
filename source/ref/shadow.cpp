...
add:
	...
	for( i = 0; i < 3; i++ ) {
		...
		maxSide = std::max( maxSide, maxs[i] - mins[i] );
	}
	...
	group->projDist = std::max( group->projDist, group->radius + std::min( r_shadows_projection_distance->value, 64.0f ) );
...
static float R_FitOccluder( const shadowGroup_t *group, refdef_t *refdef ) {
	...
	for( i = 0; i < 8; i++ ) {
		...
		x1 = std::min( x1, v[0] ); y1 = std::min( y1, v[1] ); z1 = std::min( z1, v[2] );
		x2 = std::max( x2, v[0] ); y2 = std::max( y2, v[1] ); z2 = std::max( z2, v[2] );
	}
	...
	diffx = sizex - std::min( ix1, sizex - ix2 ) * 2;
	diffy = sizey - std::min( iy1, sizey - iy2 ) * 2;
	...
	return useOrtho ? std::max( iz1, iz2 ) : group->projDist;
}
