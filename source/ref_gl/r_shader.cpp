...
static void Shader_ParseVector( const char **ptr, float *v, unsigned int size ) {
	...
	if( !token[0] ) {
		...
		const_cast<char *>( token )[strlen( token ) - 1] = 0;
		v[i] = atof( token );
	} else {
		v[i] = atof( token );
		if( bracket ) {
...
typedef struct { int operand; int op; bool negative; int val; int logic; } shaderCon_t;

static const char *const conOpStrings[] = { "<", "<=", "==", ">", ">=", "!=", NULL };
static const char *const conOpStrings2[] = { "&&", "||", NULL };
...
static void Shader_SmallestMipMapSize( shader_t *shader, shaderpass_t *pass, const char **ptr ) {
	...
	if( !r_shaderNoMipMaps ) {
		r_shaderMinMipSize = std::max( size, 1 );
	}
}
...
static void Shaderpass_AlphaGen( shader_t *shader, shaderpass_t *pass, const char **ptr ) {
	...
	if( !strcmp( token, "vertex" ) ) {
		...
		if( pass->alphagen.func.type == SHADER_FUNC_RAMP && pass->alphagen.func.args[1] == 1 ) {
			...
			shader->portalDistance = std::max( dist, shader->portalDistance );
		}
	} else if( !strcmp( token, "const" ) || !strcmp( token, "constant" ) ) {
...
static void R_LoadShaderReal( shader_t *s, const char *shortname,
							  size_t shortname_length, const char *longname, int type, const char *text ) {
	void *data;
	shaderpass_t *pass;
...
shader_t *R_LoadShader( const char *name, int type, bool forceDefault, const char *text ) {
	...
	if( nameLength + 1 > r_shortShaderNameSize || r_shortShaderNameSize < MAX_QPATH ) {
		...
		r_shortShaderNameSize = std::max( nameLength + 1, (unsigned)MAX_QPATH );
