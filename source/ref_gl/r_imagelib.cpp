...
r_imginfo_t LoadTGA( const char *name, uint8_t *( *allocbuf )( void *, size_t, const char *, int ), void *uptr ) {
	...
	if( !( targa_header.attributes & 0x20 ) ) {
		...
		uint8_t *row1, *row2;
		uint8_t *tmpLine = (uint8_t *)alloca( rowsize );
...
error:
	...
	Q_snprintfz( ver_string, sizeof( ver_string ), "%" PRIu64 ".%" PRIu64 ".%" PRIu64,
		(uint64_t)( ver / 10000 ), (uint64_t)( ( ver / 100 ) % 100 ), (uint64_t)( ver % 100 ) );
	...
	row_pointers = (unsigned char **)alloca( p_height * sizeof( *row_pointers ) );
...
void DecompressETC1( const uint8_t *in, int width, int height, uint8_t *out, bool bgr ) {
	...
	uint8_t *uncompressed = (uint8_t *)alloca( 4 * stride );
