...
static rawsound_t *S_FindRawSound( int entnum, bool addNew ) {
	...
	if( !raw_sounds[best] ) {
		raw_sounds[best] =
			(rawsound_t *)S_Malloc( sizeof( *rawsound ) + sizeof( portable_samplepair_t ) * MAX_RAW_SAMPLES );
	}
...
static void S_BeginAviDemo( void ) {
	...
	checkname = (char *)S_Malloc( checkname_size );
	...
	if( trap_FS_FOpenFile( checkname, &s_aviDumpFile, FS_WRITE ) == -1 ) {
		...
		s_aviDumpFileName = (char *)S_Malloc( checkname_size );
...
void *S_BackgroundUpdateProc( void *param ) {
	sndCmdPipe_t *s_cmdPipe = (sndCmdPipe_t *)param;
