...
sfxcache_t *SNDOGG_Load( sfx_t *s ) {
	...
	sc = s->cache = (sfxcache_t *)S_Malloc( len + sizeof( sfxcache_t ) );
	sc->length = samples;
...
bool SNDOGG_OpenTrack( bgTrack_t *track, bool *delay ) {
	...
	track->vorbisFile = vf = (OggVorbis_File *)S_Malloc( sizeof( OggVorbis_File ) );
...
#else
		read = qov_read( (OggVorbis_File *)track->vorbisFile, ( char * )ptr, (int)size, 0, 2, 1, &bs );
#endif
...
static int SNDOGG_FSeek( bgTrack_t *track, int pos ) {
	...
	return qov_pcm_seek( (OggVorbis_File *)track->vorbisFile, (ogg_int64_t)pos );
}
...
static void SNDOGG_FClose( bgTrack_t *track ) {
	if( track->vorbisFile ) {
		qov_clear( (OggVorbis_File *)track->vorbisFile ); // calls FS_FCloseFile
