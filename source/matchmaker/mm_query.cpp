...
static void StatQuery_CacheResponseRaw( stat_query_t *query ) {
	...
	query->response_raw = (char *)SQALLOC( respSize + 1 );
...
static void StatQuery_CacheTokenized( stat_query_t *query ) {
	...
	tokens = (char **)SQALLOC( ( numTokens + 1 ) * sizeof( char* ) );

	// allocate the actual buffer that we are going to return
	if( startOfs > 0 ) {
		realBuffer = (char *)SQALLOC( ( respSize - startOfs ) + 1 );
...
static stat_query_t *StatQuery_CreateQuery( const char *iface, const char *str, bool get ) {
	...
	query = (stat_query_t *)SQALLOC( sizeof( *query ) );
	memset( query, 0, sizeof( *query ) );

	if( iface && *iface ) {
		query->iface = (char *)SQALLOC( strlen( iface ) + 1 );
	...
	if( !get ) {
		...
		// add in '/', '?' and '\0' = 3
		query->url = (char *)SQALLOC( strlen( mm_url->string ) + strlen( str ) + 3 );
		// ch : lazy code :(
...
static void StatQuery_Prepare( stat_query_t *query ) {
	...
	// only allow json for POST requests
		...
		z_result = qzcompress( (Bytef *)compData, &compSize, (unsigned char*)json_text, jsonSize );
		if( z_result != Z_OK ) {
		...
		// base64
		b64Data = base64_encode( (unsigned char *)compData, compSize, &b64Size );
		if( b64Data == NULL ) {
...
static void StatQuery_SetField( stat_query_t *query, const char *name, const char *value ) {
	if( query->req ) {
		...
		query->url = (char *)SQREALLOC( query->url, len );
