...
void Steam_LoadLibrary( void ) {
	...
	funcs[1].name = NULL;
	steamlib_libhandle = (steamlib_export_t *)Com_LoadLibrary( LIB_DIRECTORY "/" LIB_PREFIX "steamlib_" ARCH LIB_SUFFIX, funcs );

	if( steamlib_libhandle ) {
		// load succeeded
		int api_version;

		steamlib_export = (steamlib_export_t *)GetSteamLibAPI( &import );
